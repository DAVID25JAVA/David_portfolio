  ...
  return (
    <>
      <div className="md:max-w-6xl px-4 sm:px-3 md:px-3 w-full mx-auto ">
        ...
        {/* Main Nav */}
        <div className="flex justify-between items-center py-3 relative z-20">
          {/* Logo */}
            ...
            <div className="hidden md:block">
              <div className="flex items-center justify-center gap-2 w-28 h-10 rounded-full text-black border-[0.5px]">
                <a href="#Contact" className="flex items-center gap-2">
                  ...
                  <img src={assets?.arrow_icon?.src} alt="arrow" className="w-3" />
                </a>
              </div>
            </div>

            {/* Hamburger Menu Icon */}
            <div onClick={() => setMobileMenu(true)} className="block md:hidden">
              <img src={assets?.menu_black?.src} alt="menu icon" className="w-8" />
            </div>
          </div>
        </div>

        {/* Mobile Menu */}
        <div className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${mobileMenu ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex justify-end items-center p-4 ">
            {/* <img className="w-20" src={assets?.logo?.src} alt="logo" /> */}
            <button onClick={() => setMobileMenu(false)}>
              <img src={assets?.close_black?.src} alt="close" className="w-5 " />
            </button>
          </div>
          <ul className="flex flex-col gap-5 p-6 text-lg">
            ...
            <li><a href="#Contact" onClick={() => setMobileMenu(false)}>Connect</a></li>
          </ul>
        </div>
      </div>
    </>
